...
class BasketOption:
    def __init__(self, S, K, r, T, σ, ρ, n, option_type, m=int(1e3), t=0.0):
    ...
    def get_normal_random(self):
        ...
        if len(self.__S)==1:
            result = np.random.randn(len(self.__S),self.__m,self.__n)
            
            
        else:
            result = np.random.randn(len(self.__S),self.__m*self.__n)
            """
            Return the Cholesky decomposition, not so sure , to be checked ,seems not correct
            """
            cholesky_result = np.linalg.cholesky(np.array(self.__ρ))
            result = np.dot(cholesky_result,result).reshape(len(self.__S),self.__m,self.__n)
    ...
    def __gen_paths(self):
        if not len(self.__S_path):
            ...
            #np.random.seed(52)
            #rand1 = np.random.randn(self.__n,self.__m)
           
            #cholesky_rand2 = np.linalg.cholesky(np.array(self.__ρ))    
            #Z = np.dot(cholesky_rand2,rand1)
            Z= self.get_normal_random()
            for i in range(len(self.__S)):
                ...
                self.__S_path.append(self.__S[i] * np.cumprod(growth_factor, 0))
    ...
    def __gen_geo_payoff(self):
        ...
        geo_mean = np.exp(np.sum(np.log(self.__S_path), axis=0) / len(self.__S))
...
basket_call = BasketOption(S=[100, 100], K=100, r=0.05, T=3, σ=[0.3, 0.3], ρ=[[1, 0.5], [0.5, 1]], n=2, option_type='Call')
print('Call Options:(By Setting Correlation = 1,Same Assets, all values converge to 1 result)')
a=basket_call.get_normal_random()
