...
class AmericanOption_BiTree:
    ...
    def get_call_tree(self):
        ...
        for i in range(self.step_num - 1):  # calculate backward the option prices
            for j in range(self.step_num - 1):
                C2[j][self.step_num - 2 - j - i] = np.exp(-self.r * self.dt) * (
                        self.up_prob * C2[j][self.step_num - j - i - 1] + self.down_prob * C2[j + 1][
                    self.step_num - 2 - j - i])
                C2[j][self.step_num - 2 - j - i] = np.maximum(C2[j][self.step_num - 2 - j - i],
                                                              C[j][self.step_num - 2 - j - i])
        return C2
    ...
    def get_put_tree(self):
        ...
        for i in range(self.step_num - 1):  # calculate backward the option prices
            for j in range(self.step_num - 1):
                P2[j][self.step_num - 2 - j - i] = np.exp(-self.r * self.dt) * (
                        self.up_prob * P2[j][self.step_num - j - i - 1] + self.down_prob * P2[j + 1][
                    self.step_num - 2 - j - i])
                P2[j][self.step_num - 2 - j - i] = np.maximum(P2[j][self.step_num - 2 - j - i],
                                                              P[j][self.step_num - 2 - j - i])
        return P2
...
if __name__ == '__main__':
    TreeTreeTree = AmericanOption_BiTree(S=50, r=0.05, sigma=0.3, T=0.25, step_num=3, K=50,
                                         option_type='call')  # Test case 1 :Lecture 7 - P.14 example

    test = TreeTreeTree.get_value()
    print("Test case 1 :Lecture 7 - P.14 example:\n", test)  # up is going right , down is going down

    TreeTreeTree = AmericanOption_BiTree(S=50, r=0.05, sigma=0.3, T=0.25, step_num=5, K=50,
                                         option_type='call')  # Test case 2 :Lecture 7 - P.15 example

    test = AmericanOption_BiTree.get_value(TreeTreeTree)
    print("Test case 2 :Lecture 7 - P.15 example:\n", test)  # up is going right , down is going down

    TreeTreeTree = AmericanOption_BiTree(S=50, r=0.05, sigma=0.223144, T=2, step_num=2, K=52,
                                         option_type='put')  # Test case 3 :Lecture 7 - P.22 example

    test = AmericanOption_BiTree.get_value(TreeTreeTree)
    print("Test case 3 :Lecture 7 - P.22 example\n", test)  # up is

    print('\n' + '-' * 10 + 'Test cases' + '-' * 10)
    tree = AmericanOption_BiTree(S=100, r=0.05, sigma=0.3, T=3, step_num=50, K=100, option_type='put')
    print('Put test case 1:\t{:.8f}'.format(tree.get_value()))
    tree = AmericanOption_BiTree(S=100, r=0.05, sigma=0.3, T=3, step_num=100, K=100, option_type='put')
    print('Put test case 2:\t{:.8f}'.format(tree.get_value()))
    tree = AmericanOption_BiTree(S=100, r=0.05, sigma=0.4, T=3, step_num=50, K=100, option_type='put')
    print('Put test case 3:\t{:.8f}'.format(tree.get_value()))

    tree = AmericanOption_BiTree(S=100, r=0.05, sigma=0.3, T=3, step_num=50, K=100, option_type='call')
    print('\nCall test case 1:\t{:.8f}'.format(tree.get_value()))
    tree = AmericanOption_BiTree(S=100, r=0.05, sigma=0.3, T=3, step_num=100, K=100, option_type='call')
    print('Call test case 2:\t{:.8f}'.format(tree.get_value()))
    tree = AmericanOption_BiTree(S=100, r=0.05, sigma=0.4, T=3, step_num=50, K=100, option_type='call')
    print('Call test case 3:\t{:.8f}'.format(tree.get_value()))
